...
import { navigate } from '../utils/router.js';   // เพิ่ม import
...
export async function render(container, params) {
  ...
  // ✅ ถ้ายังไม่ล็อกอิน ให้ redirect ไปหน้า login ทันที
  if (!currentUser) {
    navigate('/login');
    return;
  }
...
async function loadCourse(container) {
                      ...
                      <div class="py-3 space-y-3">
                        ...
                        ${lesson.external_resources ? `
                          <div class="mt-2 pt-2 border-t border-gray-800">
                            ...
                            ${lesson.external_resources.split(',').map(r => `
                              <a href="${escapeHTML(r.trim())}" target="_blank" class="text-brand hover:underline break-all block">${escapeHTML(r.trim())}</a>
                            `).join('')}
                          </div>` : ''}
  ...
  document.querySelectorAll('.lesson-toggle').forEach(toggle => {
    toggle.addEventListener('click', (e) => {
      if (e.target.closest('.mark-complete-btn')) return;
